...
edp_transactions_gitlab_project_url="${gitlab_http_url}/root/${GITLAB_EDP_PROJECT_PATH}"
edp_transactions_gitlab_ssh_clone_url="ssh://git@localhost:${GITLAB_SSH_PORT}/root/${GITLAB_EDP_PROJECT_PATH}.git"
edp_customer_gitlab_project_url="${gitlab_http_url}/root/${GITLAB_EDP_CUSTOMERS_PROJECT_PATH}"
edp_customer_gitlab_ssh_clone_url="ssh://git@localhost:${GITLAB_SSH_PORT}/root/${GITLAB_EDP_CUSTOMERS_PROJECT_PATH}.git"
...
printf '  GitLab domain transactions project: %s\n' "${edp_transactions_gitlab_project_url}"
printf '  GitLab domain customer project: %s\n' "${edp_customer_gitlab_project_url}"
printf '  Airflow UI: %s\n' "${airflow_url}"
...
printf '  Domain transactions git clone over SSH: %s\n' "${edp_transactions_gitlab_ssh_clone_url}"
printf '  Domain customer git clone over SSH: %s\n' "${edp_customer_gitlab_ssh_clone_url}"
...
printf '  dlt taxes pipeline script: %s/dlt/pipeline_taxes.py\n' "${ROOT_DIR}"
printf '  dlt depot transactions pipeline script: %s/dlt/pipeline_depot_transactions.py\n' "${ROOT_DIR}"
printf '  Source dbt project dir: %s/dbt/projects/proj_source_finnova\n' "${ROOT_DIR}"
printf '  Domain transactions dbt project dir: %s/dbt/projects/proj_domain_transactions\n' "${ROOT_DIR}"
printf '  Domain customer dbt project dir: %s/dbt/projects/proj_domain_customer\n' "${ROOT_DIR}"
...
printf '  Branch webhook endpoint: http://%s:%s/gitlab/webhook\n' "${GITLAB_BRANCH_PROVISIONER_WEBHOOK_HOST:-gitlab-branch-provisioner}" "${GITLAB_BRANCH_PROVISIONER_PORT:-8090}"
printf '  SDP rendered platform repo: %s/gitlab-projects/generated/%s\n' "${ROOT_DIR}" "${GITLAB_SDP_PROJECT_PATH}"
printf '  Domain transactions rendered platform repo: %s/gitlab-projects/generated/%s\n' "${ROOT_DIR}" "${GITLAB_EDP_PROJECT_PATH}"
printf '  Domain customer rendered platform repo: %s/gitlab-projects/generated/%s\n' "${ROOT_DIR}" "${GITLAB_EDP_CUSTOMERS_PROJECT_PATH}"
printf '\n'
...
printf '  Airflow DEV taxes DAG id: %s\n' "${DEV_TAXES_AIRFLOW_DAG_ID:-DEV_local_platform_taxes_ingest}"
printf '  Airflow DEV depot transactions DAG id: %s\n' "${DEV_DEPOT_TRANSACTIONS_AIRFLOW_DAG_ID:-DEV_local_platform_depot_transactions_ingest}"
...
printf '  Airflow PRD taxes DAG id: %s\n' "${PRD_TAXES_AIRFLOW_DAG_ID:-PRD_local_platform_taxes_ingest}"
printf '  Airflow PRD depot transactions DAG id: %s\n' "${PRD_DEPOT_TRANSACTIONS_AIRFLOW_DAG_ID:-PRD_local_platform_depot_transactions_ingest}"
...
printf '  Iceberg catalog: %s\n' "${ICEBERG_CATALOG_NAME:-default}"
printf '  Iceberg namespace: %s\n' "${ICEBERG_NAMESPACE}"
printf '  Source system slug: %s\n' "${SOURCE_SYSTEM_SLUG:-postgres}"
printf '  Object-store config prefix: %s\n' "${OBJECT_STORE_CONFIG_PREFIX:-config}"
printf '  DEV landing root: landing/dev/%s\n' "${SOURCE_SYSTEM_SLUG:-postgres}"
printf '  PRD landing root: landing/prd/%s\n' "${SOURCE_SYSTEM_SLUG:-postgres}"
printf '  Snowflake SDP database: %s\n' "${SNOWFLAKE_SDP_DATABASE}"
printf '  Snowflake SDP orders database: %s\n' "${SNOWFLAKE_SDP_ORDERS_DATABASE}"
printf '  Snowflake SDP customers database: %s\n' "${SNOWFLAKE_SDP_CUSTOMERS_DATABASE}"
printf '  Snowflake SDP taxes database: %s\n' "${SNOWFLAKE_SDP_TAXES_DATABASE}"
printf '  Snowflake SDP depot transactions database: %s\n' "${SNOWFLAKE_SDP_DEPOT_TRANSACTIONS_DATABASE}"
printf '  Snowflake SDP schemas: %s, %s, %s\n' "${SNOWFLAKE_SDP_IN_SCHEMA}" "${SNOWFLAKE_SDP_CORE_SCHEMA}" "${SNOWFLAKE_SDP_ACC_SCHEMA}"
printf '  Snowflake domain transactions database target: %s\n' "${SNOWFLAKE_EDP_ORDERS_DATABASE}"
printf '  Snowflake domain customer database target: %s\n' "${SNOWFLAKE_EDP_CUSTOMERS_DATABASE}"
printf '  Snowflake EDP taxes database target: %s\n' "${SNOWFLAKE_EDP_TAXES_DATABASE}"
printf '  Snowflake EDP depot transactions database target: %s\n' "${SNOWFLAKE_EDP_DEPOT_TRANSACTIONS_DATABASE}"
printf '  Snowflake EDP schemas: %s, %s, %s\n' "${SNOWFLAKE_EDP_IN_SCHEMA}" "${SNOWFLAKE_EDP_CORE_SCHEMA}" "${SNOWFLAKE_EDP_ACC_SCHEMA}"
printf '  Snowflake PRD SDP database: %s\n' "${PRD_SNOWFLAKE_SDP_DATABASE}"
printf '  Snowflake PRD SDP orders database: %s\n' "${PRD_SNOWFLAKE_SDP_ORDERS_DATABASE}"
printf '  Snowflake PRD SDP customers database: %s\n' "${PRD_SNOWFLAKE_SDP_CUSTOMERS_DATABASE}"
printf '  Snowflake PRD SDP taxes database: %s\n' "${PRD_SNOWFLAKE_SDP_TAXES_DATABASE}"
printf '  Snowflake PRD SDP depot transactions database: %s\n' "${PRD_SNOWFLAKE_SDP_DEPOT_TRANSACTIONS_DATABASE}"
printf '  Snowflake PRD domain transactions database target: %s\n' "${PRD_SNOWFLAKE_EDP_ORDERS_DATABASE}"
printf '  Snowflake PRD domain customer database target: %s\n' "${PRD_SNOWFLAKE_EDP_CUSTOMERS_DATABASE}"
printf '  Snowflake PRD EDP taxes database target: %s\n' "${PRD_SNOWFLAKE_EDP_TAXES_DATABASE}"
printf '  Snowflake PRD EDP depot transactions database target: %s\n' "${PRD_SNOWFLAKE_EDP_DEPOT_TRANSACTIONS_DATABASE}"
...
printf '  DEV source dbt project: %s\n' "${DEV_SNOWFLAKE_SDP_DBT_PROJECT:-DEV_DBT_PROJECT_SOURCE_FINNOVA}"
printf '  DEV domain transactions dbt project: %s\n' "${DEV_SNOWFLAKE_EDP_DBT_PROJECT:-DEV_DBT_PROJECT_DOMAIN_TRANSACTIONS}"
printf '  DEV domain customer dbt project: %s\n' "${DEV_SNOWFLAKE_EDP_CUSTOMERS_DBT_PROJECT:-DEV_DBT_PROJECT_DOMAIN_CUSTOMER}"
printf '  PRD source dbt project: %s\n' "${PRD_SNOWFLAKE_SDP_DBT_PROJECT:-PRD_DBT_PROJECT_SOURCE_FINNOVA}"
printf '  PRD domain transactions dbt project: %s\n' "${PRD_SNOWFLAKE_EDP_DBT_PROJECT:-PRD_DBT_PROJECT_DOMAIN_TRANSACTIONS}"
printf '  PRD domain customer dbt project: %s\n' "${PRD_SNOWFLAKE_EDP_CUSTOMERS_DBT_PROJECT:-PRD_DBT_PROJECT_DOMAIN_CUSTOMER}"
...
if [ -n "${GITLAB_BOOTSTRAP_PAT:-}" ] || [ -n "${GITLAB_SDP_PROJECT_ID:-}" ] || [ -n "${GITLAB_EDP_PROJECT_ID:-}" ] || [ -n "${GITLAB_EDP_CUSTOMERS_PROJECT_ID:-}" ]; then
  printf 'Generated GitLab Bootstrap Details\n'
  printf '  GitLab SDP project id: %s\n' "${GITLAB_SDP_PROJECT_ID:-<unset>}"
  printf '  GitLab domain transactions project id: %s\n' "${GITLAB_EDP_PROJECT_ID:-<unset>}"
  printf '  GitLab domain customer project id: %s\n' "${GITLAB_EDP_CUSTOMERS_PROJECT_ID:-<unset>}"
  printf '  GitLab bootstrap PAT: %s\n' "${GITLAB_BOOTSTRAP_PAT:-<unset>}"
  printf '  GitLab SDP runner token: %s\n' "${GITLAB_SDP_RUNNER_TOKEN:-<unset>}"
  printf '  GitLab domain transactions runner token: %s\n' "${GITLAB_EDP_RUNNER_TOKEN:-<unset>}"
  printf '  GitLab domain customer runner token: %s\n' "${GITLAB_EDP_CUSTOMERS_RUNNER_TOKEN:-<unset>}"
