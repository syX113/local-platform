...
edp_customers_gitlab_project_url="${gitlab_http_url}/root/${GITLAB_EDP_CUSTOMERS_PROJECT_PATH}"
edp_customers_gitlab_ssh_clone_url="ssh://git@localhost:${GITLAB_SSH_PORT}/root/${GITLAB_EDP_CUSTOMERS_PROJECT_PATH}.git"
...
printf '  GitLab EDP orders project: %s\n' "${edp_gitlab_project_url}"
printf '  GitLab EDP customers project: %s\n' "${edp_customers_gitlab_project_url}"
printf '  Airflow UI: %s\n' "${airflow_url}"
...
printf '  EDP orders git clone over SSH: %s\n' "${edp_gitlab_ssh_clone_url}"
printf '  EDP customers git clone over SSH: %s\n' "${edp_customers_gitlab_ssh_clone_url}"
...
printf '  EDP orders rendered platform repo: %s/gitlab-projects/generated/%s\n' "${ROOT_DIR}" "${GITLAB_EDP_PROJECT_PATH}"
printf '  EDP customers rendered platform repo: %s/gitlab-projects/generated/%s\n' "${ROOT_DIR}" "${GITLAB_EDP_CUSTOMERS_PROJECT_PATH}"
printf '\n'
...
printf '  Snowflake EDP orders database target: %s (not materialized after initialization; branch/MR CI uses isolated suffixed databases)\n' "${SNOWFLAKE_EDP_DATABASE}"
printf '  Snowflake EDP customers database target: %s (deployed during initialization)\n' "${SNOWFLAKE_EDP_CUSTOMERS_DATABASE}"
...
printf '  Snowflake PRD EDP orders database target: %s (not materialized after initialization; branch/MR CI uses isolated suffixed databases)\n' "${PRD_SNOWFLAKE_EDP_DATABASE}"
printf '  Snowflake PRD EDP customers database target: %s (deployed during initialization)\n' "${PRD_SNOWFLAKE_EDP_CUSTOMERS_DATABASE}"
...
printf '  DEV EDP orders dbt project: %s (deployed during initialization; executes into isolated branch/MR databases until CD deploy)\n' "${DEV_SNOWFLAKE_EDP_DBT_PROJECT:-DEV_DBT_PROJECT_EDP_ORDERS}"
printf '  DEV EDP customers dbt project: %s (deployed during initialization)\n' "${DEV_SNOWFLAKE_EDP_CUSTOMERS_DBT_PROJECT:-DEV_DBT_PROJECT_EDP_CUSTOMERS}"
printf '  PRD source dbt project: %s\n' "${PRD_SNOWFLAKE_SDP_DBT_PROJECT:-PRD_DBT_PROJECT_SOURCE_FINNOVA}"
printf '  PRD EDP orders dbt project: %s (deployed during initialization; PRD database is created only after PRD CD deploy)\n' "${PRD_SNOWFLAKE_EDP_DBT_PROJECT:-PRD_DBT_PROJECT_EDP_ORDERS}"
printf '  PRD EDP customers dbt project: %s (deployed during initialization)\n' "${PRD_SNOWFLAKE_EDP_CUSTOMERS_DBT_PROJECT:-PRD_DBT_PROJECT_EDP_CUSTOMERS}"
