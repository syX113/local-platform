...
- Four Airflow DAGs for PostgreSQL -> MinIO/Iceberg ingestion:
  - `orders`
  - `customers`
  - `taxes`
  - `depot_transactions`
- Four dlt ingestion entrypoints:
  - `orders`
  - `customers`
  - `taxes`
  - `depot_transactions`
- One combined source dbt project for Snowflake `INBOUND`, `CORE`, and `ACCESS`, executed natively in Snowflake
- Source dbt manifests are published to MinIO so the downstream domain repositories can weave the public source models into their own dbt projects with dbt-loom
...
- The clone lifecycle is driven by `ci/snowflake/data_products.json`, so every registered Snowflake data product database is cloned, not just the sample products.
...
- PRD deployment runs in the same post-merge pipeline after the approval gate and deploys to shared `PRD_` targets such as `PRD_local_platform_orders_ingest`, `PRD_local_platform_customers_ingest`, `PRD_local_platform_taxes_ingest`, and `PRD_local_platform_depot_transactions_ingest`.
