...
ARTIFACT_DIR="${ROOT_DIR}/artifacts/sdp-prd-cd"
rm -rf "${ARTIFACT_DIR}"
mkdir -p "${ARTIFACT_DIR}"

source_sdp_database="${SNOWFLAKE_SDP_DATABASE}"
source_edp_database="${SNOWFLAKE_EDP_DATABASE}"
clone_owner_token="SDP_PRD"
clone_branch_token="prd_${CI_PIPELINE_ID:-local}_${CI_COMMIT_SHORT_SHA:-head}"
prd_slug="$(sanitize_branch_token "prd_${CI_PROJECT_PATH_SLUG:-proj_sdp_orders}_${CI_PIPELINE_ID:-local}_${CI_COMMIT_SHORT_SHA:-head}")"
prd_slug="${prd_slug:-local}"
prd_hash="$(stable_token "${prd_slug}")"
prd_namespace="$(printf '%s' "${prd_slug}" | cut -c1-29)_${prd_hash}"
prd_namespace="$(sanitize_branch_token "${prd_namespace}")"
prd_namespace="$(trim_identifier "${prd_namespace}" 40)"
prd_namespace="${prd_namespace:-local}"
